...
import kotlinx.android.synthetic.main.warehouse_fragment.*
...
class WarehouseFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        firstName = customView!!.findViewById(R.id.firstName)
        lastName = customView.findViewById(R.id.lastName)
        email = customView.findViewById(R.id.email)
        company = customView.findViewById(R.id.company)
        ...
        dialog = AlertDialog.Builder(mContext)
                ...
                .setView(customView)
                .setPositiveButton("CREATE", DialogInterface.OnClickListener { dialogInterface, _ ->
                    ...
                    if(!Patterns.EMAIL_ADDRESS.matcher(email.text).matches() or
                        ...
                        createAttendantUrl.httpPost(params).header("Authorization" to "Bearer "+loggedInUser.token).timeout(BaseUtils(mContext).TIMEOUT).responseString { request, response, result ->
                            pDialog.dismiss()
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when (item?.itemId){
            R.id.action_add -> {
                ...
                warehouseForm.visibility = View.VISIBLE
                getAttendants(true)
            }
        }
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        getExistingWarehouses();

        val addWarehouse = view!!.findViewById<Button>(R.id.addWarehouse)
        ...
        addWarehouse.setOnClickListener {
            shopList.visibility = View.GONE
            emptyState.visibility = View.INVISIBLE
            warehouseForm.visibility = View.VISIBLE
            getAttendants(true)
        ...
        registerBtn.setOnClickListener {
            pDialog = ProgressDialog.show(mContext, "Please wait", "Creating warehouse...", false, false);
            ...
            params.add(Pair("is_warehouse", "1"))
            ...
            createShopUrl.httpPost(params).header("Authorization" to "Bearer "+loggedInUser.token).timeout(BaseUtils(mContext).TIMEOUT).responseString { request, response, result ->
                ...
                if(err != null){
                    mContext.toast("Unable to create warehouse. Please try again")
                    return@responseString
                }
    ...
    private fun getAttendants(proceedToNextStep : Boolean) {
        ...
        attendantUrl.httpGet().header("Authorization" to "Bearer "+loggedInUser.token).timeout(BaseUtils(mContext).TIMEOUT).responseString { request, response, result ->
            ...
            val (res, err) = result
            if(err != null){
                mContext.toast("Unable to get attendants")
                return@responseString
            }
    ...
    private fun getLinesOfBusiness() {
        ...
        lobUrl.httpGet().header("Authorization" to "Bearer "+loggedInUser.token).timeout(BaseUtils(mContext).TIMEOUT).responseString { request, response, result ->
            ...
            val (res, err) = result
            if(err != null){
                mContext.toast("Unable to get lines of business. Please try again")
                return@responseString
            }
    ...
    private fun getExistingWarehouses() {
        pDialog = ProgressDialog.show(mContext, null, "Getting existing warehouses...", false, false);

        val shopsUrl = getString(R.string.base_url)+"stores/?is_warehouse=1"
        ...
        shopsUrl.httpGet().header("Authorization" to "Bearer "+loggedInUser.token).timeout(BaseUtils(mContext).TIMEOUT).responseString { request, response, result ->
            ...
            val (res , err) = result
            if(err != null){
                mContext.toast("Unable to get existing warehouses. Please try again")
                return@responseString
            }
            ...
            try {
                ...
                if(existingShops.size == 0){
                    ...
                    warehouseForm.visibility = View.INVISIBLE
